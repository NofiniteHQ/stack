...
export const ToolbarButton = ({
  ...
  <motion.button
    ...
    className={cn(
      ...
      disabled &&
        'opacity-50 cursor-not-allowed hover:bg-transparent hover:text-muted'
    )}
    onClick={onClick}
    disabled={disabled}
    aria-label={label}
...
export const LinkButton = ({ editor }: { editor: TiptapEditor }) => {
  ...
  return (
    <Popover>
      ...
      <Popover.Content
        placement="bottom"
        className="flex gap-2 p-2 bg-surface border border-default rounded-lg shadow-xl"
      >
        <Input
          type="url"
          value={url}
  ...
  return (
    <>
      <Popover>
        ...
        <Popover.Content
          placement="bottom"
          className="flex flex-col gap-2 w-64 p-3 bg-surface border border-default rounded-lg shadow-xl"
        >
          ...
          <div className="text-[10px] font-medium text-center text-muted uppercase tracking-wider my-1">
            OR
          </div>
          <Popover.Close>
            <Button
    ...
    <Dropdown>
      <Dropdown.Trigger>
        <button
          ...
          className="flex-shrink-0 flex items-center justify-center gap-1 h-8 w-10 rounded-md hover:bg-subtle text-muted hover:text-default transition-colors border-none bg-transparent outline-none focus-visible:outline-none"
        >
...
const FontFamilyDropdown = ({ editor }: { editor: TiptapEditor }) => {
  ...
  const activeFont = fonts.find((f) => currentFontFamily === f.value)?.label;
...
const TableCreator = ({ editor }: { editor: TiptapEditor }) => {
  ...
  return (
    <Popover>
      ...
      <Popover.Content
        placement="bottom"
        className="p-3 bg-surface border border-default rounded-lg shadow-xl flex flex-col gap-2"
      >
