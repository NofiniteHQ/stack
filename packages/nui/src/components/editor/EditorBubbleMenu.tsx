...
import { LinkButton } from './EditorToolbar';
...
export const EditorBubbleMenu: React.FC<EditorBubbleMenuProps> = ({
  ...
  const baseImgClass =
    'max-w-full h-auto rounded-md shadow-sm my-4 transition-all duration-200';
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <BubbleButton
            ...
            label="Float Right"
            onClick={() =>
              setImgClass(`${baseImgClass} w-1/2 float-right ml-6`)
            }
          />
        </motion.div>
      </BubbleMenu>
      ...
      <BubbleMenu
        ...
        shouldShow={({ editor }) =>
          editor.isActive('customVideo') || editor.isActive('youtube')
        }
      >
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 5 }}
      ...
      <BubbleMenu
        ...
        shouldShow={({ editor, state }) => {
          ...
          if (empty || editor.isActive('image') || editor.isActive('table'))
            return false;
          return true;
        }}
      >
