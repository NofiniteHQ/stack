...
export const BlockMathNodeView: React.FC<NodeViewProps> = ({
  node,
  selected,
  updateAttributes,
}) => {
  ...
  useEffect(() => {
    if (katexRef.current) {
      try {
        katex.render(
          equation || '\\text{Enter an equation...}',
          katexRef.current,
          {
            displayMode: true,
            throwOnError: true,
  ...
  return (
    <NodeViewWrapper
      className={cn(
        'my-4 relative rounded-lg border border-default bg-surface shadow-sm overflow-hidden',
    ...
    >
      ...
      <div
        className={cn(
          'flex justify-center items-center min-h-[80px] p-6 bg-surface overflow-x-auto',
