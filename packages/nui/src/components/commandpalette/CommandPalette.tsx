...
export function CommandPalette({
 ...
 <input
 ref={inputRef}
 className="grow bg-transparent px-3 py-4 text-lg text-default outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus placeholder:text-muted"
 placeholder={placeholder}
 value={query}
 onChange={(e) => setQuery(e.target.value)}
 />
 {query && (
 <button className="rounded bg-subtle px-1.5 py-0.5 text-[10px] font-semibold text-muted border border-default focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus" onClick={() => setQuery('')}>
 ESC
 </button>
 )}
 </div>
 ...
 {sec.title && (
 <div className="px-3 py-2 text-[12px] font-semibold uppercase tracking-wider text-muted">{sec.title}</div>
 )}
 ...
 className={cn("flex cursor-pointer items-center gap-3 rounded-md p-3 text-default transition-colors", isActive && "bg-subtle text-default font-medium")}
 ...
 <kbd key={k} className="flex min-w-[20px] items-center justify-center rounded border border-default bg-surface px-1 text-[10px] font-semibold text-muted shadow-[0_1px_1px_rgba(0,0,0,0.05)]">{key}</kbd>
 ))}
 </div>
 )}
 </div>
