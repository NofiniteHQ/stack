  ...
  return (
    <Portal>
      ...
      <div
        ref={paletteRef}
        className={cn(
          'command-palette-box fixed left-1/2 top-[15%] z-[99999] flex w-full max-w-2xl -translate-x-1/2 flex-col overflow-hidden rounded-xl border border-default bg-surface backdrop-blur-md shadow-2xl animate-in fade-in slide-in-from-top-4 font-sans',
          className
        )}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-center gap-2 border-b border-default px-4">
          ...
          <input
            ref={inputRef}
            className="command-input flex-1 min-w-0 w-full bg-transparent px-2 py-4 text-lg text-default border-none outline-none ring-0 shadow-none focus:border-none focus:outline-none focus:ring-0 focus:shadow-none placeholder:text-muted"
          ...
          {query && (
            <button
              className="shrink-0 rounded bg-subtle px-2 py-1 text-[10px] font-semibold text-muted border border-default outline-none focus:outline-none"
              onClick={() => setQuery('')}
            >
              ESC
            </button>
        ...
        {/* Results List */}
        <div
          className="max-h-[400px] overflow-y-auto p-2 scroll-smooth"
          role="listbox"
        >
          {filteredSections.length === 0 ? (
            ...
            filteredSections.map((sec, sIdx) => (
              <div key={sIdx} className="mb-2">
                {sec.title && (
                  <div className="px-4 py-2 text-xs font-semibold uppercase tracking-wider text-primary mb-1">
                    {sec.title}
                  </div>
                )}
                  ...
                  {sec.items.map((item, iIdx) => {
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          'command-item flex cursor-pointer items-center gap-3 rounded-xl px-4 py-3 transition-colors duration-200',
                          isActive
                            ? 'bg-subtle text-default font-medium shadow-sm'
                            : 'text-default hover:bg-subtle/50'
                        )}
                        onMouseEnter={() =>
                        ...
                        {item.shortcut && (
                          <div className="hidden ml-auto md:flex items-center gap-1">
                            {item.shortcut.split(' ').map((key, k) => (
                              <Kbd
                                key={k}
                                className="flex min-w-[24px] items-center justify-center rounded-md border border-default bg-surface px-1.5 py-0.5 text-[11px] font-semibold text-muted shadow-sm"
                              >
                                {key}
                              </Kbd>
                            ))}
