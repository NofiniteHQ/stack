...
export const ContainerSizes: StoryObj = {
  render: () => (
    <Stack gap={16} className="font-sans text-sm">
      <Container size="sm">
        <div className="bg-subtle border border-default rounded-md px-6 py-4 text-center font-medium text-default">
          SM Container
        </div>
      </Container>
      <Container size="md">
...
/* ---------- Flex Stories ---------- */
const boxClass =
  'px-6 py-3 bg-surface border border-default rounded-md text-default font-medium text-sm shadow-sm';
...
export const FlexPlayground: StoryObj = {
  render: () => (
    <Flex
      ...
      justify="between"
      align="center"
      className="p-6 border border-default border-dashed rounded-lg bg-subtle font-sans"
    >
  ...
  render: () => (
    <Grid gap={16} minColWidth="150px" className="font-sans">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="p-6 border border-default bg-surface rounded-md text-default text-center font-medium text-sm shadow-sm"
        >
          Item {i + 1}
        </div>
      ))}
