...
import { DataTree, DataTreeNode } from './DataTree';

const mockData: DataTreeNode[] = [
 {
 id: '1',
 label: 'Documents',
 children: [
...
describe('TreeView Component', () => {
 ...
 const { container } = render(<DataTree nodes={mockData} />);
 expect(await axe(container)).toHaveNoViolations();
 });

 it('renders root nodes but hides children by default', () => {
 render(<DataTree nodes={mockData} />);
 ...
 const user = userEvent.setup();
 render(<DataTree nodes={mockData} defaultExpandedIds={['1']} />);
 ...
 render(<DataTree nodes={mockData} onSelect={onSelectSpy} />);
