...
describe('Pagination Component', () => {
  ...
  it('renders correct page numbers for small totals without ellipses', () => {
    ...
    expect(
      document.querySelector('li[aria-hidden="true"]')
  ...
  it('renders ellipses for large totals to prevent UI overflow', () => {
    ...
    render(
      <Pagination page={50} total={100} siblings={1} onChange={vi.fn()} />
    );
    ...
    expect(
      screen.getByRole('button', { name: 'Go to page 100' })
    ).toBeInTheDocument();

    expect(
      screen.getByRole('button', { name: 'Go to page 49' })
    ).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Page 50' })).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: 'Go to page 51' })
    ).toBeInTheDocument();

    // Should not see far away pages
    expect(
      screen.queryByRole('button', { name: 'Go to page 20' })
    ).not.toBeInTheDocument();
  ...
  it('supports currentPage, totalPages, and onPageChange prop aliases', async () => {
    const user = userEvent.setup();
