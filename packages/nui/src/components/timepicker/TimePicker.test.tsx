...
describe('TimePicker Component', () => {
  ...
  it.skip('renders placeholder initially', () => {
    render(<TimePicker placeholder="Pick a time" />);
    expect(screen.getByText('Pick a time')).toBeInTheDocument();
  });

  it.skip('opens panel on click', async () => {
  ...
  it.skip('selects a time and calls onChange sequentially', async () => {
    const user = userEvent.setup();
  ...
  it.skip('correctly pads single digits when formatting the label', () => {
    // In 24h format, 09:05 stays 09:05
    render(<TimePicker value="09:05" clockType={24} />);
    expect(screen.getByText('09:05')).toBeInTheDocument();
  });

  it.skip('correctly formats 12h AM/PM strings for display', () => {
