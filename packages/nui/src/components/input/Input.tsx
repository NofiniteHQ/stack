...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    return (
      <div
        className={cn('flex flex-col gap-1 w-full font-sans', wrapperClassName)}
      >
        {label && (
          <label
            htmlFor={inputId}
            className={cn(
              'text-sm font-medium text-default leading-tight',
              disabled && 'text-muted'
            )}
          >
            {label}
        ...
        <div className="relative w-full flex items-center">
          {leftIcon && (
            <span className="absolute left-3 flex items-center justify-center text-muted pointer-events-none">
              {leftIcon}
            </span>
          )}
          ...
          <input
            ...
            className={cn(
              ...
              isInvalid &&
                'border-danger focus-visible:border-danger focus-visible:shadow-[inset_0_0_0_1px_var(--nui-color-danger)]',
              !!leftIcon && 'pl-[40px]',
              !!rightIcon && 'pr-[40px]',
              className
            )}
          ...
          {rightIcon && (
            <span
              className={cn(
                'absolute right-3 flex items-center justify-center text-muted pointer-events-none',
                rightIconClassName
              )}
            >
              {rightIcon}
        ...
        {error && (
          <div
            id={errorId}
            className="text-[13px] text-danger font-medium mt-1"
            aria-live="polite"
          >
            {error}
          </div>
