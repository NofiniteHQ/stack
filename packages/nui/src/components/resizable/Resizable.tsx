...
const ResizableRoot = forwardRef<HTMLDivElement, ResizableProps>(
  (
    { direction = 'horizontal', onLayout, className, children, ...props },
    ref
  ) => {
 ...
 ---------------------------------------------------- */
    const resizePanels = useCallback(
      (handleIndex: number, deltaPct: number) => {
        ...
        const maxDeltaRight = Math.min(
          maxA - startSizes[handleIndex],
          startSizes[handleIndex + 1] - minB
        );
        const maxDeltaLeft = Math.max(
          minA - startSizes[handleIndex],
          startSizes[handleIndex + 1] - maxB
        );

        // Clamp the requested delta to the allowed range
        const allowedDelta = Math.max(
          maxDeltaLeft,
          Math.min(maxDeltaRight, deltaPct)
        );
        ...
        if (allowedDelta !== 0) {
          ...
          handles[handleIndex]?.setAttribute(
            'aria-valuenow',
            startSizes[handleIndex].toFixed(0)
          );
          return true;
        }
        return false;
 ...
 ---------------------------------------------------- */
    const startDrag = useCallback(
      (handleElement: HTMLElement, e: React.PointerEvent) => {
        e.preventDefault();
        ...
        const availablePx = isHorizontal
          ? container.clientWidth
          : container.clientHeight;

        // We must subtract the size of the handles themselves so the math calculates percentage of *free* space
        const totalHandlePx = handles.reduce(
          (acc, h) => acc + (isHorizontal ? h.offsetWidth : h.offsetHeight),
          0
        );
 ...
 ---------------------------------------------------- */
    const keyboardResize = useCallback(
      (handleElement: HTMLElement, e: React.KeyboardEvent) => {
        const container = containerRef.current;
        if (!container) return;
 ...
 ---------------------------------------------------- */
    useLayoutEffect(() => {
      ...
      if (currentSizes.current.length !== panels.length) {
        const initial = panels.map((p) =>
          parseFloat(p.dataset.defaultSize || '0')
        );
    ...
    return (
      <ResizableContext.Provider
        value={{ direction, startDrag, keyboardResize }}
      >
        <div
          ref={(el) => {
...
const ResizablePanel = forwardRef<HTMLDivElement, ResizablePanelProps>(
  (
    { defaultSize, minSize = 0, maxSize = 100, className, children, ...props },
    ref
  ) => {
    return (
      <div
        ...
        className={cn('min-w-0 min-h-0 overflow-hidden relative', className)}
        {...props}
      >
        {children}
      </div>
...
const ResizableHandle = forwardRef<HTMLDivElement, ResizableHandleProps>(
  ({ withIcon = false, className, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            'absolute z-[2] inset-0 m-auto bg-[var(--border-default)] transition-all group-hover:bg-primary data-[dragging=true]:bg-primary',
        ...
        {withIcon && (
          <div
            className={cn(
              'absolute z-[3] inset-0 m-auto rounded-full bg-[var(--border-strong)] transition-all group-hover:bg-primary group-hover:scale-110 data-[dragging=true]:bg-primary data-[dragging=true]:scale-110',
