...
export const StatCard = forwardRef<HTMLDivElement, StatCardProps>(
  (
    ...
    const accentStyles = {
      ...
      brand: 'border-primary/30 hover:border-primary/60',
    };
    ...
    return (
      <Card
        ref={ref}
        className={cn(
          'stat-card overflow-hidden font-sans group relative transition-all duration-200 !p-6',
          accentStyles[accent],
          className
        )}
        ...
        <div className="flex items-center justify-between space-x-4">
          <div className="flex flex-col space-y-1 z-10 w-full">
            ...
            <div className="text-3xl font-semibold text-default tracking-tight pt-0.5">
              {isLoading ? (
                <Skeleton.Text width="60%" size="xl" className="mt-1" />
              ) : (
                value
              )}
            </div>
          ...
          >
            {(trendValue || trend) && (
              ...
              >
                ...
                {isNeutral && (
                  <Minus className="w-3.5 h-3.5 mr-1" strokeWidth={2.5} />
                )}
                {trendValue}
              </span>
            )}
            {trendLabel && (
              <span
                className="text-sm text-muted font-medium truncate"
                aria-hidden="true"
              >
                {trendLabel}
              </span>
