...
export const InteractiveWizard: Story = {
  render: function Wizard() {
    ...
    const handleNext = () =>
      setActive((prev) => Math.min(prev + 1, detailedSteps.length - 1));
    ...
    return (
      ...
      >
        <Stepper data={detailedSteps} active={active} onChange={setActive} />
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Button
            variant="primary"
            onClick={handleNext}
            disabled={active === detailedSteps.length - 1}
          >
            Next
          </Button>
        </div>
...
export const AutomatedTest: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    ...
    // Check initial state
    const firstStep = canvas.getByText(/Shipping/i).closest('button');
...
export const DataDrivenContent: Story = {
  render: function Wizard() {
    ...
    const smartSteps = [
      {
        ...
        content: (
          <div className="p-8 border rounded-lg bg-surface mt-4 text-center">
      ...
      },
      {
        ...
        content: (
          <div className="p-8 border rounded-lg bg-surface mt-4 text-center text-primary font-bold">
    ...
    return (
      ...
      >
        <Stepper data={smartSteps} active={active} onChange={setActive} />
        <div
          style={{
            display: 'flex',
        ...
        >
          ...
          <Button
            variant="primary"
            onClick={() =>
              setActive(Math.min(active + 1, smartSteps.length - 1))
            }
            disabled={active === smartSteps.length - 1}
          >
            Next
          </Button>
        </div>
