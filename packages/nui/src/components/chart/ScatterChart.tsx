        ...
        <AxisBottom
          ...
          tickFormat={(val) => {
            ...
            if (Number(val) > 1000000000000) {
              return new Date(Number(val)).toLocaleDateString(undefined, {
                month: 'short',
                day: 'numeric',
              });
            }
        ...
        >
          <div className="text-muted text-xs mb-1 font-medium uppercase tracking-wider">
            {Number(xAccessor(tooltipData)) > 1000000000000
              ? new Date(Number(xAccessor(tooltipData))).toLocaleDateString(
                  undefined,
...
export function ScatterChart<T>(
  props: Omit<ScatterChartProps<T>, 'width' | 'height'>
) {
  return (
    <div
