...
function BaseLineChart<T>({
  ...
  const yScale = useMemo(
    () =>
      scaleLinear({
        ...
        domain: [
          0,
          (max(data, yAccessor) || 0) + (max(data, yAccessor) || 0) * 0.2,
        ],
        nice: true,
      }),
  ...
  return (
    <div
      ...
      className="text-default font-sans"
    >
      ...
      {tooltipData && (
        <TooltipWithBounds
          ...
          left={tooltipLeft}
          style={tooltipStyles}
          className="bg-glass backdrop-blur-sm text-default z-50"
        >
          <div className="text-muted text-xs mb-1 font-medium uppercase tracking-wider">
            {xValue(tooltipData).toLocaleDateString(undefined, {
              year: 'numeric',
              month: 'short',
              day: 'numeric',
            })}
...
export function LineChart<T>(
  props: Omit<LineChartProps<T>, 'width' | 'height'>
) {
  return (
    <div
      style={{ width: '100%', height: '100%', minHeight: 300, minWidth: 300 }}
    >
      <ParentSize>
        {({ width, height }) => (
          <BaseLineChart width={width} height={height} {...props} />
        )}
      </ParentSize>
    </div>
  );
